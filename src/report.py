#!/usr/bin/env python3
"""
NecroTrace - Official Forensic Post-Mortem Report Generator.
Modeled after official Police & Medico-Legal Post-Mortem Examination Forms (Form No. 5372 / Autopsy Dossier).
Integrates real autopsy schedules, external/internal postmortem findings, metagenomic succession typing,
probable time-since-death windows, and official medical officer certification.
"""
...
def generate_forensic_timeline_chart(
    ...
    """
    fig, ax = plt.subplots(figsize=(6.8, 1.6), dpi=220)
...
def generate_forensic_pdf(
    ...
    # -------------------------------------------------------------
    institution = case_metadata.get("institution", "Central Medico-Legal Examination Center & Morgue")
    ...
    deceased_name = case_metadata.get("deceased_name", "Unidentified Male (Ref: Unknown Deceased #42)")
    guardian = case_metadata.get("guardian_name", "Not Known")
    address = case_metadata.get("address", "Recovered from woodland edge, Sector-5")
    ...
    clothes = case_metadata.get("clothes", "Soiled cotton shirt, dark trousers; unfastened, torn at knees")
    marks = case_metadata.get("id_marks", "Old linear surgical scar (4 cm) on right forearm; tattoo 'Om' on right wrist")
    ...
    # Real clinical autopsy findings mapped to decomposition timeline
    if pred_days < 2.0:
        rigor_desc = "Rigor mortis present in lower extremities, waning in facial muscles."
        ext_desc = "Hypostasis fixed on dependent posterior surfaces; greenish discoloration starting in right iliac fossa."
    elif pred_days < 10.0:
        rigor_desc = "Rigor mortis completely passed off (absent throughout body)."
        ext_desc = "Greenish-black discoloration over abdomen and thorax; prominent superficial venous marbling ('arborescent pattern'); abdominal distension with putrefactive gases (bloat); postmortem skin slippage on hands and feet; blood-stained frothy purge fluid at nares and mouth."
    else:
        rigor_desc = "Rigor mortis absent. Extensive tissue lysis."
        ext_desc = "Advanced decomposition with extensive soft tissue liquefaction, black putrefaction, partial skeletonization of facial bones, maggot crawling over exposed orifices."
    ...
    obs_rows = [
        ...
        [
            ...
            Paragraph(rigor_desc, field_val),
    ...
    # Clarified Specimen Adequacy (explaining what sequencing yield means)
    if read_depth >= 1000:
    ...
    # Backwards time calculation for date of death window
    try:
    ...
    opinion_rows = [
        [
            ...
            Paragraph(
                "Based on integrated evaluation of physical decomposition changes (cessation of rigor, extent of putrefactive marbling/bloat) "
                "concordant with calibrated high-throughput metagenomic bacterial succession signatures under prevailing ambient temperatures.",
                field_val
            ),
        ],
        [
            Paragraph("<b>2. CAUSE OF DEATH:</b>", field_label),
            Paragraph("ASPHYXIA AS A RESULT OF CONSTRICTION OF NECK (PENDING CHEMICAL & HISTOPATHOLOGICAL CONFIRMATION).", field_val_bold),
            Paragraph("<b>3. MANNER:</b>", field_label),
            Paragraph("Homicidal / Matter under judicial investigation.", field_val),
        ]
    ]
    ...
    img = Image(chart_buf, width=7.4 * inch, height=1.5 * inch)
    story.append(img)
    story.append(Spacer(1, 4))
    ...
    specimen_rows = [
        ...
        [
            ...
            Paragraph("Cotton shirt, pants, belt sealed in brown packet.", field_val),
    ...
    designation = case_metadata.get("designation", "Medical Officer & Forensic Metagenomics Specialist")
    hosp_dept = case_metadata.get("department", "District Medico-Legal Center / Government Hospital")

    sig_table_data = [
        [
            Paragraph(
                "<b>Certified to be True Copy</b><br/><br/>"
...
if __name__ == "__main__":
    sample_case = {
        "pm_report_no": "PM-619 / 2026",
        "police_station": "New Township P.S.",
        "inquest_no": "14 / 2026",
        "inquest_date": "15 / 09 / 2026",
        "institution": "District Medico-Legal Center & Morgue",
        "analyst": "Dr. Tanish Walture",
        "reg_no": "Reg. No. 45826 of WBMC",
        "designation": "Medical Officer & Forensic Specialist",
        "department": "Department of Forensic Medicine, Team BroomWroom",
        "deceased_name": "Unidentified Male (Found near railway track)",
        "guardian_name": "Not Known",
        "address": "Found at Sector-5 Woodland Perimeter",
        "age": "Approx. 35 - 40 Years",
        "sex": "Male",
        "police_officials": "ASI R. K. Mandal, New Township P.S.",
        "relatives_identified": "Identified by Police Inquest Paper #14/26",
        "receipt_time": "15/09/2026, 09:30 hrs",
        "commence_time": "15/09/2026, 10:45 hrs",
        "complete_time": "15/09/2026, 12:15 hrs",
        "inquest_exam_time": "15/09/2026, 08:00 hrs",
        "height_cm": "172 cm",
        "weight_kg": "68 kg",
        "physique": "Average built, moderately nourished",
        "clothes": "Soiled cotton shirt, dark trousers; unfastened, torn at knees",
        "id_marks": "Old linear scar (4 cm) on right forearm; tattoo 'Om' on wrist",
        "sample_id": "SAMP_0042",
        "sample_site": "Oral Cavity & Nasal Mucosal Swab",
        "raw_data_hash": compute_sha256_hash("Sample_0042_Necrobiome_Counts_RAW"),
    }
    sample_pmi = {
        "predicted_pmi": 6.84,
        "lower_bound": 5.72,
        "upper_bound": 8.01,
        "decomposition_stage": "Bloat / Active Decomposition",
        "ambient_temp_c": 23.5,
    }
    sample_qc = {
        "read_depth": 28410,
        "shannon_entropy": 2.914,
        "retained_taxa": 44,
        "initial_taxa": 50,
        "dropped_taxa": 6,
    }

    test_out = "artifacts/sample_forensic_report.pdf"
    generate_forensic_pdf(sample_case, sample_pmi, sample_qc, output_path=test_out)
